...
void i8259_SetMask(uint16_t newMask) {
  ...
  iowait();
  dprintf("[\e[0;32mPIC\e[0m] Set mask to 0x%08llX\n", g_PicMask);
}

uint16_t i8259_GetMask() {
  uint16_t mask = inb8(PIC1_DATA) | (inb8(PIC2_DATA) << 8);
  dprintf("[\e[0;32mPIC\e[0m] Get mask: 0x%08llX\n", mask);
  return mask;
}

void i8259_Configure(uint8_t offsetPic1, uint8_t offsetPic2, bool autoEoi) {
  dprintf("[\e[0;32mPIC\e[0m] Configuring PIC with offsets 0x%04llX and "
          "0x%04llX, AutoEOI: %s\n",
...
void i8259_SendEndOfInterrupt(int irq) {
  ...
  // dprintf("[\e[0;32mPIC\e[0m] Sending End of Interrupt for IRQ %d\n", irq);
  if (irq >= 8)
...
void i8259_Disable() {
  dprintf("[\e[0;31mPIC\e[0m] Disabling PIC\n");
  i8259_SetMask(0xFFFF);
}

void i8259_Enable() {
  dprintf("[\e[0;32mPIC\e[0m] Enabling PIC\n");
...
void i8259_Mask(int irq) {
  dprintf("[\e[0;32mPIC\e[0m] Masking IRQ %d\n", irq);
  i8259_SetMask(g_PicMask | (1 << irq));
}

void i8259_Unmask(int irq) {
  dprintf("[\e[0;32mPIC\e[0m] Unmasking IRQ %d\n", irq);
  i8259_SetMask(g_PicMask & ~(1 << irq));
}

uint16_t i8259_ReadIrqRequestRegister() {
  dprintf("[\e[0;32mPIC\e[0m] Reading IRQ Request Register\n");
  outb8(PIC1_COMMAND, PIC_CMD_READ_IRR);
  outb8(PIC2_COMMAND, PIC_CMD_READ_IRR);
  return ((uint16_t)inb8(PIC2_COMMAND)) | (((uint16_t)inb8(PIC2_COMMAND)) << 8);
}

uint16_t i8259_ReadInServiceRegister() {
  dprintf("[\e[0;32mPIC\e[0m] Reading In-Service Register\n");
